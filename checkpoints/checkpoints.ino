...
void loop()
{
    ...
    if (readsuccess)
    {
        ...
        http.begin("http://192.168.8.190/pathfinder/getBoardData"); // Specify request destination
        ...
        sendData(serverURL, UIDresultSend);
    }
}
...
void sendData(const char* serverURL, String data){
  HTTPClient http;
  ...
  int code = http.POST(sendingData);
  if(code > 0){
    Serial.println("Data Sent Successfully");
    Serial.println(http.getString());
  } else {
    Serial.println("Cannot Send Data. Check the IP Address and Network");
  }
}
